  ...
  render() {
    ...
    const comment = (v: any, children: any, fId: any) => (
      <Comment
        ...
        datetime={
          <Tooltip title={moment(v.create_at).format("YYYY-MM-DD HH:mm:ss")}>
            <span>{moment(v.create_at).fromNow()}</span>
          </Tooltip>
        }
      >
        {children
          ? children.map((v: any) => (
              <div key={v.id}>{comment(v, v.children, fId)}</div>
            ))
          : null}
      </Comment>
    );
